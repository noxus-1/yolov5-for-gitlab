    ...
    def load_dataset(self):
        ...
        for defect_type in defect_types:
            if defect_type == 'good':
                ...
                gt_paths.extend(glob.glob(os.path.join(self.img_path, defect_type) + "/*.jpg"))
                img_paths.sort()
                gt_paths.sort()
                img_tot_paths.extend(img_paths)
...
class PatchCore(pl.LightningModule):
    def __init__(self, hparams):
        ...
        self.input_size ,self.load_size = args.input_size,args.load_size 
        self.init_features()
        def hook_t(module, input, output):
            self.features.append(output)
    
        self.model = torch.hub.load('pytorch/vision:v0.8.2', 'resnet18', pretrained=True)
    ...
    def test_step(self, batch, batch_idx): # Nearest Neighbour Search
        ...
        # import pdb
        # pdb.set_trace()
        anomaly_map = score_patches[:,0].reshape((int(self.input_size/8),int(self.input_size/8)))
